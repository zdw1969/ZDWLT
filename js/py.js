...
function setupAudioContext() {
    ...
    if (isChromium) {
        document.getElementById("playera").addEventListener('play', function () {
            if (!cx) {
                try {
                    ...
                    console.error("Web Audio API 初始化失败，请更新您的浏览器。");
                }
            }
        }, { once: true }); // 确保只在第一次播放时初始化
    } else {
        console.warn("非 Chromium 内核浏览器，跳过 Web Audio API。");
    }
}
